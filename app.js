...
async function renderPageGrid(){
  ...
  for(const p of state.pages){
    ...
    const el=document.createElement('button'); el.type='button'; el.className=`page-thumb ${p.selected?'selected':''} ${state.pages[state.currentPage]?.id===p.id?'current':''}`;
    ...
    el.addEventListener('click',async e=>{
      p.selected=e.shiftKey ? !p.selected : p.selected;
...
function updateCompareVisual(){
  const pct=Number(state.compare); const clip=100-pct;
  if(refs.afterCanvas){refs.afterCanvas.style.clipPath=`inset(0 0 0 ${clip}%)`;refs.compareHandle.style.left=`${pct}%`;refs.compareLine.style.left=`${pct}%`;}
  if(refs.compareText) refs.compareText.textContent=`${pct}% printable view`;
}
