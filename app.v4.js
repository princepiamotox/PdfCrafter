...
const refs = {
  ...
  beforeCanvas:q('#beforeCanvas'), afterCanvas:q('#afterCanvas'), compareSlider:q('#compareSlider'), compareHandle:q('#compareHandle'), compareLine:q('#compareLine'),
  ...
  rows:q('#gridRows'), cols:q('#gridCols'), rowsOut:q('#gridRowsOut'), colsOut:q('#gridColsOut'), gridCount:q('#gridCount'), gridPreset:q('#gridPreset'),
...
function syncInputs(){
  ...
  refs.rows.value=state.rows; refs.cols.value=state.cols; refs.rowsOut.textContent=state.rows; refs.colsOut.textContent=state.cols; refs.gridCount.textContent=`${sheetCount()} slides / A4`;
  refs.compareSlider.value=state.compare; updateCompareVisual();
}

function applyPreset(name){
  if(!PRESETS[name]) return;
  state.filters={...PRESETS[name]}; syncInputs(); refreshPreviews();
}
...
async function renderPageGrid(){
  ...
  for(const p of state.pages){
    ...
    const el=document.createElement('div'); el.className=`page-thumb ${p.selected?'selected':''} ${state.pages[state.currentPage]?.id===p.id?'current':''}`;
    ...
    el.querySelector('.thumb-preview').addEventListener('click',async ()=>{
      ...
      renderPageGrid(); renderFiles(); updatePageMeta(); await renderSourcePreview();
    });
    el.querySelector('.thumb-select').addEventListener('click',async e=>{
      e.stopPropagation(); p.selected=!p.selected; state.currentPage=state.pages.findIndex(x=>x.id===p.id);
      renderPageGrid(); renderFiles(); updatePageMeta(); renderSummary(); renderSheetMiniPreview(); await refreshPreviews();
    });
    el.querySelector('.thumb-delete').addEventListener('click',async e=>{
      e.stopPropagation(); await deletePage(p.id);
    });
    el.addEventListener('contextmenu',e=>{e.preventDefault();p.selected=!p.selected;renderPageGrid();renderSummary();renderSheetMiniPreview();});
    refs.pageGrid.appendChild(el);
    try{
      const img=await renderSourceToCanvas(p,360,{applyFilters:false}); const c=el.querySelector('canvas'); const scale=Math.min(1,270/img.width);
...
async function renderSourcePreview(){
  ...
  try{
    const before=await renderSourceToCanvas(p,1500,{applyFilters:false});
    const after=await renderSourceToCanvas(p,1500,{applyFilters:true});
    const maxW=Math.max(320,refs.previewStage.clientWidth-48), maxH=Math.max(280,refs.previewStage.clientHeight-56);
    ...
    refs.beforeCanvas.width=refs.afterCanvas.width=w; refs.beforeCanvas.height=refs.afterCanvas.height=h;
    ...
    refs.previewTitle.textContent=state.activePreview==='source'?`PDF page ${p.pageNumber} — Before / After`:'A4 output preview';
...
function updateCompareVisual(){
  ...
  if(refs.afterCanvas){
    refs.afterCanvas.style.clipPath=`inset(0 0 0 ${clip}%)`;
    refs.compareHandle.style.left=`${pct}%`;
    refs.compareLine.style.left=`${pct}%`;
  }
...
async function renderSourceToCanvas(p,targetWidth=1400,opts={applyFilters:true}){
  ...
  await page.render({canvasContext:ctx,viewport:vp}).promise; return opts.applyFilters?applyFilters(c,state.filters):c;
}
...
async function drawA4Sheet(ctx,W,H,selected,docMode=false){
  ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H); const {rows,cols,margin,gap,cellW,cellH}=cellGeometry(W,H); const batch=selected.slice(0,sheetCount());
  for(let i=0;i<batch.length;i++){
    const img=await renderSourceToCanvas(batch[i],docMode?1800:1100,{applyFilters:true}),scale=Math.min(cellW/img.width,cellH/img.height),dw=img.width*scale,dh=img.height*scale;
...
async function drawSheetOnCanvas(canvas,compact=false){
  const selected=selectedPages();if(!selected.length)return; const [W,H]=compact?previewDimensions():[canvas.width,canvas.height]; canvas.width=W;canvas.height=H;await drawA4Sheet(canvas.getContext('2d'),W,H,selected,false);
}
async function previewSheet(){
  const selected=selectedPages();if(!selected.length)return alert('Select at least one page.'); const [W,H]=previewDimensions(); refs.sheetCanvas.width=W;refs.sheetCanvas.height=H;await drawA4Sheet(refs.sheetCanvas.getContext('2d'),W,H,selected,false);
  refs.sheetDialogTitle.textContent=`A4 ${state.orientation} · ${state.rows} × ${state.cols} · ${Math.min(sheetCount(),selected.length)} slide${Math.min(sheetCount(),selected.length)===1?'':'s'} on sheet`; refs.sheetDialog.showModal();
}
function renderSheetMiniPreview(){const c=q('#miniA4Canvas');if(!c||!selectedPages().length)return;const [W,H]=previewDimensions();c.width=W/2;c.height=H/2;const ctx=c.getContext('2d');ctx.save();ctx.scale(.5,.5);drawA4Sheet(ctx,W,H,selectedPages(),false).catch(()=>{});ctx.restore();}

async function renderSheetTab(){
  const selected=selectedPages(); if(!selected.length)return;
  const [W,H]=previewDimensions();refs.previewCanvas.width=W;refs.previewCanvas.height=H;refs.previewCanvas.classList.remove('hidden');
  await drawA4Sheet(refs.previewCanvas.getContext('2d'),W,H,selected,false); refs.previewTitle.textContent='A4 output preview';
  refs.beforeCanvas.classList.add('hidden');refs.afterCanvas.classList.add('hidden');refs.compareHandle.classList.add('hidden');refs.compareLine.classList.add('hidden');
}

async function refreshPreviews(){
  if(state.activePreview==='sheet') await renderSheetTab(); else {refs.beforeCanvas.classList.remove('hidden');refs.afterCanvas.classList.remove('hidden');refs.compareHandle.classList.remove('hidden');refs.compareLine.classList.remove('hidden');await renderSourcePreview();}
  ...
  try{
    ...
    for(let gi=0;gi<groups.length;gi++){
      ...
      for(let i=0;i<groups[gi].length;i++){
        const p=groups[gi][i],targetPx=Math.min(2600,Math.max(700,Math.round(cellW*state.quality/72))),canvas=await renderSourceToCanvas(p,targetPx,{applyFilters:true});
...
async function deletePage(pageId){
  ...
  renderPageGrid(); renderFiles(); updateSourceInfo(); updatePageMeta(); renderSummary(); await refreshPreviews();
}
function removeSelectedPages(){
  ...
  renderPageGrid(); renderFiles(); updateSourceInfo(); updatePageMeta(); renderSummary(); refreshPreviews();
}
...
function setActivePreview(mode){state.activePreview=mode;refs.sourcePreviewTab.classList.toggle('active',mode==='source');refs.sheetPreviewTab.classList.toggle('active',mode==='sheet');refreshPreviews();}
...
function changeGrid(){state.rows=Number(refs.rows.value);state.cols=Number(refs.cols.value);refs.gridPreset.value='custom';syncInputs();updateLayoutHint();renderSummary();renderSheetMiniPreview();if(state.activePreview==='sheet')refreshPreviews();}
...
function bind(){
  ...
  refs.selectAllBtn.addEventListener('click',()=>{state.pages.forEach(p=>p.selected=true);renderPageGrid();renderSummary();refreshPreviews();});
  ...
  refs.gridPreset.addEventListener('change',e=>{const v=e.target.value;if(v==='custom')return;const [r,c]=v.split('x').map(Number);state.rows=r;state.cols=c;syncInputs();updateLayoutHint();renderSummary();refreshPreviews();});
  ...
  ['brightness','contrast','background','sharpness'].forEach(id=>refs[id].addEventListener('input',()=>{state.filters[id]=Number(refs[id].value);refs[id+'Out'].textContent=refs[id].value;refreshPreviews();}));
  ['grayscale','invert','autoLight','pageNumbers'].forEach(id=>refs[id].addEventListener('change',()=>{state.filters[id]=refs[id].checked;refreshPreviews();}));
  refs.margin.addEventListener('input',()=>{state.margin=Number(refs.margin.value);refs.marginOut.textContent=`${state.margin} mm`;updateLayoutHint();renderSummary();renderSheetMiniPreview();});
  ...
  refs.resetFiltersBtn.addEventListener('click',()=>{state.filters={...PRESETS.clean};refs.presetSelect.value='clean';syncInputs();refreshPreviews();});
  ...
  const moveCompare=(clientX)=>{
    if(!refs.compareStage)return;
    const r=refs.compareStage.getBoundingClientRect();
    const pct=Math.max(0,Math.min(100,((clientX-r.left)/r.width)*100));
    state.compare=Math.round(pct); updateCompareVisual();
  };
  refs.compareHitarea?.addEventListener('pointerdown',e=>{draggingCompare=true;refs.compareHitarea.setPointerCapture?.(e.pointerId);moveCompare(e.clientX);});
  refs.compareHitarea?.addEventListener('pointermove',e=>{if(draggingCompare)moveCompare(e.clientX);});
  refs.compareHitarea?.addEventListener('pointerup',()=>{draggingCompare=false;});
  refs.compareHitarea?.addEventListener('pointercancel',()=>{draggingCompare=false;});
  refs.compareHitarea?.addEventListener('click',e=>moveCompare(e.clientX));
  ...
  refs.dropZone.addEventListener('drop',e=>handleFiles(e.dataTransfer.files));window.addEventListener('resize',()=>{if(state.pages.length)refreshPreviews();});
}
